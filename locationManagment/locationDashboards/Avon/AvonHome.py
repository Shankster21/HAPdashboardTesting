...
curs.execute("select *  from filelib.haptempsv2 where hc_site_name = 'Avon'")
rows = curs.fetchall()

curs.execute("select distinct hc_bin_code from filelib.haptempsv2 where hc_site_name = 'Avon' order by hc_bin_code ")
bins = curs.fetchall()

curs.execute("select distinct hc_read_date from filelib.haptempsv2 where hc_site_name = 'Avon'")
dates = curs.fetchall()

# curs.execute("select distinct hc_read_date from filelib.haptempsv2 where hc_site_name = 'Avon' and hc_read_date = '{}'").formate(today)
# todayDate = curs.fetchall()

curs.execute("select distinct hc_bin_code from filelib.haptempsv2 where hc_site_name = 'Avon'")
HAPbins = curs.fetchall()

curs.execute("select distinct gb_bcbin, gb_capacity, gb_bushels_in_bin, gb_bin_will_hold, gb_commodity_code, gb_measure_date from filelib.getbinclct where  gb_location = '23' order by gb_bcbin ")
binMeasurements = curs.fetchall()

# curs.execute("select distinct hc_bin_code from filelib.haptempsv2 where hc_site_name = 'Avon' and avgf > '80' and hc_read_date = '2020-08-08'")
# hotBins = curs.fetchall()

# _________ WRITE HTML FILE ________
with open('AvonHomePROD.html','w') as file:

# HTML page head info stuff
    file.write('''<!doctype html><html lang="en">
    <head><title>Avon Home Page</title><meta charset="utf-8"><meta name="description" content="The HTML5 Herald"><meta name="author" content="SitePoint">
    ...
    <link rel="stylesheet" type="text/css" href="AvonHome.css"><script src="AvonHome.js"></script>\n</head>\n\n''')
    ...
    file.write('''<header>
         ...
         <div class="row no-gutters">
              ...
              <div class="col-5 col2">
                   <div id = "locationInfo">
                             <p> Location Number: 23 </p>
                           <p> Functional Unit: 12 </p>

                   </div>
                   <!-- <div id = "diagram">
                   <img src="AvonDiagram.pdf" > -->
...
with open('AvonHome.js','w') as file:
