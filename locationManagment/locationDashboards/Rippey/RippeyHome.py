...
curs.execute("select *  from filelib.haptempsv2 where hc_site_name = 'Rippey'")
rows = curs.fetchall()

curs.execute("select distinct hc_bin_code from filelib.haptempsv2 where hc_site_name = 'Rippey' order by hc_bin_code ")
bins = curs.fetchall()

curs.execute("select distinct hc_read_date from filelib.haptempsv2 where hc_site_name = 'Rippey'")
dates = curs.fetchall()

# curs.execute("select distinct hc_read_date from filelib.haptempsv2 where hc_site_name = 'Rippey' and hc_read_date = '{}'").formate(today)
# todayDate = curs.fetchall()

curs.execute("select distinct hc_bin_code from filelib.haptempsv2 where hc_site_name = 'Rippey'")
HAPbins = curs.fetchall()

curs.execute("select distinct gb_bcbin, gb_capacity, gb_bushels_in_bin, gb_bin_will_hold, gb_commodity_code, gb_measure_date from filelib.getbinclct where  gb_location = '30' order by gb_bcbin ")
binMeasurements = curs.fetchall()

# curs.execute("select distinct hc_bin_code from filelib.haptempsv2 where hc_site_name = 'Rippey' and avgf > '80' and hc_read_date = '2020-08-08'")
# hotBins = curs.fetchall()

# _________ WRITE HTML FILE ________
with open('RippeyHomePROD.html','w') as file:

# HTML page head info stuff
    file.write('''<!doctype html><html lang="en">
    <head><title>Rippey Home Page</title><meta charset="utf-8"><meta name="description" content="The HTML5 Herald"><meta name="author" content="SitePoint">
    ...
    <link rel="stylesheet" type="text/css" href="RippeyHome.css"><script src="RippeyHome.js"></script>\n</head>\n\n''')
    ...
    file.write('''<header>
         ...
         <div class="row no-gutters">
              ...
              <div class="col-5 col2">
                   <div id = "locationInfo">
                             <p> Location Number: 30 </p>
                           <p> Functional Unit: 15 </p>

                   </div>
                   <!-- <div id = "diagram">
                   <img src="RippeyDiagram.pdf" > -->
...
with open('RippeyHome.js','w') as file:
